#include "EQLNES_Sim900.h"
#include <Arduino.h>
#include <String.h>

void EQLNES_Sim900::begin(Stream *serial)
{
    _serial = serial;
    _powerStatus = POWER_OFFLINE;
}

void EQLNES_Sim900::togglePower()
{
...
bool EQLNES_Sim900::ensureReady()
{
    return _ensurePowerStatus(POWER_READY, MAX_POWER_RETRIES);
}

bool EQLNES_Sim900::ensureOffline()
{
    return _ensurePowerStatus(POWER_OFFLINE, MAX_POWER_RETRIES);
}

bool EQLNES_Sim900::_ensurePowerStatus(int status, int maxRetries)
{
    bool success = false;
    int retries = 0;
...
bool EQLNES_Sim900::isReady()
{
    return _powerStatus == POWER_READY;
}

bool EQLNES_Sim900::isOffline()
{
    return  _powerStatus == POWER_OFFLINE;
}

bool EQLNES_Sim900::isUnknownState()
{
    return  _powerStatus == POWER_UNKNOWN;
}

void EQLNES_Sim900::waitPowerToggleCompleted()
{
    waitPowerToggleCompleted(DEFAULT_POWER_TIMEOUT);
}

void EQLNES_Sim900::waitPowerToggleCompleted(int timeout)
{
...
bool EQLNES_Sim900::isTextMsgDelivered()
{
    return isTextMsgDelivered(DEFAULT_SMS_TIMEOUT);
}

bool EQLNES_Sim900::isTextMsgDelivered(int timeout)
{
    char smsOK[] = "OK";
    int smsOKLength = 2;
    int numOKNeeded = 2;
...
void EQLNES_Sim900::clearBuffer()
{
    char c;
...
void EQLNES_Sim900::sendTextMsg(char *msg, char *phoneNumber)
{
...
void EQLNES_Sim900::sendTextMsg(String msg, char *phoneNumber)
{
    _serial->print(F("AT+CMGF=1\r"));
    delay(500);
...
int EQLNES_Sim900::read()
{
    return _serial->read();
}

int EQLNES_Sim900::available()
{
    return _serial->available();
}

void EQLNES_Sim900::writeAtCommand(String command)
{
